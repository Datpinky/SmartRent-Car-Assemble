...
class UploadValidation {
  ...
  validateImageUpload = [
    body('files').custom((value, { req }) => {
      ...
      if (!files.length) {
        throw new Error('Cần ít nhất một file (form-data, field: files, tối đa 10 file)');
      }
  ...
  validateVehicleDamageImages = [
    body('before_rental').custom((value, { req }) => {
      ...
      if (!before || !before.length) {
        throw new Error('Cần đúng 1 ảnh xe trước khi cho thuê (form-data, field: before_rental)');
      }
      if (before.length > 1) {
        throw new Error('Chỉ gửi 1 file cho before_rental');
      }
      if (!before[0].mimetype?.startsWith('image/')) {
        throw new Error(`${before[0].originalname || 'file'} không phải ảnh`);
      }
      return true;
    }),
    body('after_return').custom((value, { req }) => {
      ...
      if (!after || !after.length) {
        throw new Error('Cần đúng 1 ảnh xe khi trả (form-data, field: after_return)');
      }
      if (after.length > 1) {
        throw new Error('Chỉ gửi 1 file cho after_return');
      }
      if (!after[0].mimetype?.startsWith('image/')) {
        throw new Error(`${after[0].originalname || 'file'} không phải ảnh`);
      }
      return true;
    }),
  ];
