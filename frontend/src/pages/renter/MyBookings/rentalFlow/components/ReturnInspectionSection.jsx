...
const normalizeImages = (value) => {
  ...
  if (value && typeof value === 'object') return Object.values(value).flat().filter(Boolean).slice(0, MAX_RETURN_IMAGES);
  return [];
};
...
const ReturnInspectionSection = ({ workflow, onChangeReturnImages, returnLocked }) => {
  ...
  const canAddMore = returnImages.length < MAX_RETURN_IMAGES;
  ...
  const handleImageUpload = async (event) => {
    ...
    const remainingSlots = MAX_RETURN_IMAGES - returnImages.length;
    const acceptedFiles = files.filter((file) => file.type.startsWith('image/')).slice(0, remainingSlots);
    if (!acceptedFiles.length) return;

    setUploading(true);
    try {
      const uploadedUrls = await uploadService.uploadImages(acceptedFiles);
      const newImages = uploadedUrls.map((u) => u.url || u).filter(Boolean);
      onChangeReturnImages([...returnImages, ...newImages].slice(0, MAX_RETURN_IMAGES));
    } catch (err) {
    ...
    <div style={BASE_CARD_STYLE}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, color: '#111827' }}>
            Ảnh trả xe
            {returnImages.length > 0 && <FaCheckCircle style={{ display: 'inline', marginLeft: 8, color: '#10b981' }} />}
          </div>
      ...
      >
        {returnImages.map((url, idx) => (
          ...
          >
            <img src={url} alt={`Ảnh trả xe ${idx + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            {!returnLocked && (
              <button
                type="button"
        ...
        {!returnLocked && canAddMore && (
          ...
          >
            <FaPlus />
            <span style={{ fontSize: '0.78rem' }}>{uploading ? 'Đang tải...' : 'Thêm ảnh'}</span>
          </button>
        )}
      </div>
