...
const AdminDashboard = () => {
  ...
  if (error) {
    return (
      <div
        role="alert"
        style={{ padding: 32, textAlign: 'center', color: '#dc2626', background: '#fff', borderRadius: 14, margin: 20 }}
      >
        {error}
      </div>
    );
  ...
  return (
    <div className="admin-dash">
      ...
      <div className="stats-grid">
        ...
        <StatCard
          title="Tổng Showroom"
          value={stats?.totalShowrooms?.toLocaleString() ?? '—'}
          icon={<FaStore />}
          color="#0891b2"
        />
        <StatCard
          title="Tổng lượt đặt xe"
      ...
      <div className="charts-grid">
        ...
        <div className="chart-card">
          <div className="chart-header">
            <div className="chart-title">Trạng thái xe</div>
          </div>
          {vehicleStatusPie.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: 40, color: '#9ca3af', fontSize: '0.85rem' }}>
              Chưa có dữ liệu
            </div>
          )}
        </div>
      ...
      <div className="chart-card" style={{ marginTop: 20 }}>
        <div className="chart-header">
          <div className="chart-title">Tăng trưởng người dùng (6 tháng gần nhất)</div>
        </div>
        <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="chart-card" style={{ marginTop: 20 }}>
        <div className="chart-header">
          <div className="chart-title">Phân loại xe (theo loại)</div>
        </div>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={vehicleCategoryPie} dataKey="value" cx="50%" cy="50%" outerRadius={88} paddingAngle={3}>
                  {vehicleCategoryPie.map((entry, i) => (
                    <Cell key={i} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip formatter={(value, name) => [value + ' xe', name]} />
      ...
      <div className="section-card" style={{ marginTop: 20 }}>
        <div className="section-header">
          ...
          <button type="button" className="btn-link" onClick={() => navigate('/admin/transactions')}>
            Xem tất cả <FaEye aria-hidden="true" />
          </button>
        </div>
              ...
              <tbody>
                {recentBookings.map((b) => (
                  <tr key={String(b.id)}>
                    ...
                    <td>{b.from}</td>
                    <td>{b.to}</td>
                    <td className="tabular-nums" style={{ fontWeight: 600, color: '#00b14f' }}>
                      {formatVnd(b.total)}
                    </td>
                    <td>
