...
const LoginFormField = ({
  ...
  <div className="flex flex-col gap-1.5">
    ...
    <div className="relative flex items-center">
      <Icon aria-hidden="true" className="pointer-events-none absolute left-3 text-gray-400" size={17} />
      <input
        id={`field-${name}`}
...
const Login = () => {
  ...
  const registerPhoneInvalid =
    tab === 'register' && !!registerError && (form.phone || '').replace(/\D/g, '').length !== 10;
  ...
  const handleSubmit = async (event) => {
    ...
    if (tab === 'register') {
      if (!passwordMeetsPolicy(form.password)) {
        setRegisterError('Mật khẩu chưa đủ độ mạnh. Vui lòng đáp ứng đầy đủ các yêu cầu bên dưới ở mật khẩu.');
        return;
      }
      ...
      setSubmitting(true);
      const result = await register(form.name, form.email, form.password, form.phone, form.accountType || 'renter');
      setSubmitting(false);
    ...
    if (result.success) {
      ...
      const redirect =
        from && from !== '/login' && allowedPrefix && String(from).startsWith(allowedPrefix) ? from : fallback;
            ...
            {['login', 'register'].map((value) => (
              <button
                ...
                className={`-mb-0.5 flex-1 border-b-2 py-2.5 text-center text-[0.9rem] font-semibold transition-all ${
                  tab === value ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'
                }`}
                onClick={() => {
                  setForgotInlineOpen(false);
                  setTab(value);
        ...
        {!forgotInlineOpen && (
          <div className="mt-5 space-y-2 text-center text-[0.83rem] text-gray-500">
            {tab === 'login' ? (
              <>
                Chưa có tài khoản?{' '}
                <button type="button" className="font-semibold text-primary" onClick={() => setTab('register')}>
                  Đăng ký ngay
                </button>
              </>
            ) : (
              <>
                Đã có tài khoản?{' '}
                <button type="button" className="font-semibold text-primary" onClick={() => setTab('login')}>
                  Đăng nhập
                </button>
              </>
            )}
